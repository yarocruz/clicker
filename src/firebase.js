...
import { getFirestore, collection, getDocs, updateDoc, addDoc, query, where } from 'firebase/firestore/lite';
...
export async function updateClicks(country, city) {
    ...
    const countryDoc = querySnapshot.docs.find(doc =>
        (doc.data().name === country && doc.data().city === city) || (doc.data().name === null && doc.data().city === null)
    );
